...
function CommandPalette({ role, onClose }: { role: AdminRole; onClose: () => void }) {
  ...
  const quickActions = useMemo<PaletteItem[]>(
    () => [
      ...
      { key: "q:broadcast", label: tCmd("quick.sendBroadcast"), Icon: Send, run: () => go("/admin/whatsapp") },
      { key: "q:whatsapp", label: tCmd("quick.openWhatsapp"), Icon: MessageCircle, run: () => go("/admin/whatsapp/chats") },
    ],
    [tCmd, go],
  );
